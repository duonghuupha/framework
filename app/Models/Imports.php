...
class Imports extends Model{
    ...
    public static function listImports(array $params = []): array{
        ...
        return self::paginateAdv(static::$view_import, $params);
    }
     ...
     * THÊM CHI TIẾT
     * ========================================================== */
    private static function insertItems(int $importId, array $products): void{
        foreach ($products as $item) {
            $detail = [
                'import_id' => $importId,
                'product_id' => $item['id'],
            ...
            if (!$result) {
                throw new Exception("Không lưu được chi tiết phiếu nhập.");
            }
        }
    }
     ...
     * ========================================================== */
    private static function increaseStock(int $productId, float $qty, float $importPrice): void {
        $sql = "
            UPDATE products
            SET
                stock = stock + ?,
            ...
            WHERE id = ?
        ";
        $result = self::execQuery($sql, [$qty, $importPrice, $productId]);
        if (!$result) {
